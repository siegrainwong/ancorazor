/* ======================== Modified: Scroll sync extension BEGIN  */
...
function scrollSyncExtension(editor) {
	...
	var scrollManager = new _scrollManager2.default(
		sectionManager,
		cm,
		editor.preview.$el
	);
	...
	function changeButtonVisiblityStateIfNeed() {
		if (
			editor.mdPreviewStyle === "vertical" &&
			editor.currentMode === "markdown"
		) {
			button.$el.show();
			$divider.show();
		} else {
	...
	editor.eventManager.listen("scroll", function(event) {
		...
		if (isScrollable && editor.preview.isVisible()) {
			if (
				event.source === "markdown" &&
				!scrollManager.isMarkdownScrollEventBlocked
			) {
				scrollManager.syncPreviewScrollTopToMarkdown();
			} else if (
				event.source === "preview" &&
				!scrollManager.isPreviewScrollEventBlocked
			) {
...
_editorProxy2.default.defineExtension(
	"scrollSync",
	scrollSyncExtension
);
...
/* ======================== Modified: Scroll sync extension END  */